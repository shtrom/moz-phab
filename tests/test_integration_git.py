...
def test_submit_create_no_checkout(in_process, git_repo_path: pathlib.Path, init_sha):
    ...
    assert (
        "Differential Revision" in first_log
    ...
    second_log = git_out("log", "--format=%s%n%n%b", "-2", "second").strip()
    assert (
        "Differential Revision" in second_log
    ), "The commits in the first branch should contain the Differential Revision."
...
def test_uplift(in_process, git_repo_path: pathlib.Path, init_sha):
    call_conduit.reset_mock()
    call_conduit.side_effect = [
        # diffusion.repository.search
        {
